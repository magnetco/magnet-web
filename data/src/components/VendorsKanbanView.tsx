...
import { fetchAll, updateRecord, deleteRecord, createRecord, Vendor, VendorStatus } from '../lib/api'
...
interface VendorsKanbanViewProps {
  onRecordClick: (id: number) => void
}

const statusConfig: { status: VendorStatus; label: string; color: string; bgColor: string }[] = [
  { status: 'prospect', label: 'Prospect', color: 'text-violet-700', bgColor: 'bg-violet-50 border-violet-200' },
  { status: 'evaluating', label: 'Evaluating', color: 'text-sky-700', bgColor: 'bg-sky-50 border-sky-200' },
  { status: 'approved', label: 'Approved', color: 'text-amber-700', bgColor: 'bg-amber-50 border-amber-200' },
  { status: 'active', label: 'Active', color: 'text-emerald-700', bgColor: 'bg-emerald-50 border-emerald-200' },
  { status: 'on_hold', label: 'On Hold', color: 'text-orange-700', bgColor: 'bg-orange-50 border-orange-200' },
  { status: 'inactive', label: 'Inactive', color: 'text-slate-500', bgColor: 'bg-slate-50 border-slate-200' },
]

const searchFields = ['name', 'contact_name', 'email', 'service_type']

export default function VendorsKanbanView({ onRecordClick }: VendorsKanbanViewProps) {
  const [vendors, setVendors] = useState<Vendor[]>([])
  const [loading, setLoading] = useState(true)
  const [draggedVendor, setDraggedVendor] = useState<Vendor | null>(null)
  const [dragOverColumn, setDragOverColumn] = useState<VendorStatus | null>(null)
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const result = await fetchAll<Vendor>('vendors')
      setVendors(result)
    } catch (err) {
  ...
  const handleDragStart = (e: React.DragEvent, vendor: Vendor) => {
    ...
    e.dataTransfer.setData('text/plain', String(vendor.id))
  }
  ...
  const handleDragOver = (e: React.DragEvent, status: VendorStatus) => {
    e.preventDefault()
  ...
  const handleDrop = async (e: React.DragEvent, newStatus: VendorStatus) => {
    e.preventDefault()
    setDragOverColumn(null)

    if (!draggedVendor || draggedVendor.status === newStatus) return

    // Optimistically update UI
    setVendors((prev) =>
      prev.map((v) =>
        v.id === draggedVendor.id ? { ...v, status: newStatus } : v
      )
    )

    try {
      await updateRecord('vendors', draggedVendor.id, 'status', newStatus)
    } catch (err) {
  ...
  const handleDelete = async (e: React.MouseEvent, vendorId: number) => {
    e.stopPropagation()
    if (!confirm('Are you sure you want to delete this vendor?')) return
    await deleteRecord('vendors', vendorId)
    await loadData()
  }
  ...
  const handleCreate = async () => {
    setCreating(true)
    try {
      const newVendor = await createRecord<Vendor>('vendors', {
        name: 'New Vendor',
        status: 'prospect',
      })
      onRecordClick(newVendor.id)
    } catch (err) {
    ...
    return [
      {
        key: 'service_type',
        label: 'Service',
        options: getUniqueValues(vendors as unknown as Record<string, unknown>[], 'service_type'),
      },
    ]
  }, [vendors])
  ...
  const filteredVendors = useMemo(() => {
    return vendors.filter((vendor) => {
      const rec = vendor as unknown as Record<string, unknown>
  ...
  const displayVendors = filteredVendors.slice(0, MAX_RECORDS)
  const displayedCount = displayVendors.length

  const getVendorsByStatus = (status: VendorStatus) =>
    displayVendors.filter((v) => v.status === status)
    ...
    <div className="bg-frost rounded-lg border border-opal shadow-sm">
      ...
      <div className="flex flex-col gap-4 px-4 py-3 border-b border-opal">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h2 className="font-medium text-oxblood">Vendors</h2>
            <span className="text-sm text-basalt">
              {displayedCount === totalCount
                ? `${totalCount.toLocaleString()} vendor${totalCount !== 1 ? 's' : ''}`
                : `${displayedCount.toLocaleString()} of ${totalCount.toLocaleString()} vendors`}
            </span>
          </div>
          <button
      ...
      <div className="grid grid-cols-6 gap-3 p-4 min-h-[500px]">
        {statusConfig.map(({ status, label, color, bgColor }) => {
          const columnVendors = getVendorsByStatus(status)
                ...
                {columnVendors.length === 0 ? (
                  ...
                  columnVendors.map((vendor) => (
                    <div
                      ...
                      onClick={() => onRecordClick(vendor.id)}
