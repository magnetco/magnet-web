...
export function AnimatedTextLogo({ 
  ...
  const animateLogo = useCallback(() => {
    ...
    const clipRects = clipPathsRef.current
    const timeline = gsap.timeline({ ease: 'power2.out' })

    // Calculate bounding boxes and set up clip paths
    paths.forEach((path, index) => {
      const bbox = path.getBBox()
      const rect = clipRects[index]
      const group = groupsRef.current[index]
      if (rect && group) {
        // Set the clip rect to match the path's bounding box
        rect.setAttribute('x', String(bbox.x))
        rect.setAttribute('y', '0')
        rect.setAttribute('width', String(bbox.width))
        rect.setAttribute('height', '27')
        rect.setAttribute('data-width', String(bbox.width))
        
        // Reset to hidden (width = 0, opacity = 0)
        gsap.set(rect, {
          width: 0,
        })
        gsap.set(group, {
    ...
    // Animate each character with overlapping timing - reveal fill like a marker drawing across
    const charDuration = 0.12 // Each character takes 0.12s (shorter)
    const overlapOffset = 0.05 // More overlap - each character starts 0.05s after previous

    clipRects.forEach((rect, index) => {
      const fullWidth = parseFloat(rect.getAttribute('data-width') || '0')
      const group = groupsRef.current[index]
      const startTime = index * overlapOffset
      ...
      timeline.to(
        ...
        {
          opacity: 1,
          duration: charDuration * 0.6, // Fade in over 60% of character duration
          ease: 'power2.out',
        },
        startTime,
      )
      
      // Reveal the fill from left to right (like a marker drawing across the full width)
      timeline.to(
        rect,
        {
          width: fullWidth,
        ...
        startTime, // Same start time for overlap
      )
    })
  }, [])
  ...
  useEffect(() => {
    // Detect when scrolling back to top
    ...
    const handleScroll = () => {
      ...
      // If we were scrolled and now we're back at the top, re-animate
      if (wasScrolled && isAtTop) {
        animateLogo()
        wasScrolled = false
        ...
        <defs>
          {characters.map((char, index) => (
            <clipPath key={`clip-${index}`} id={`clip-${index}`}>
              <rect
                ref={(el) => {
                  if (el) clipPathsRef.current[index] = el
                }}
                x="0"
                y="0"
                width="0"
