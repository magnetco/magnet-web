    ...
    <tbody>
      <tr>
        <th
          ...
          scope="colgroup"
          className="border-t border-b border-t-olive-950/5 border-b-olive-950/10 pt-14 pb-4 font-semibold text-oxblood"
        >
          {group.title}
        </th>
      </tr>
  ...
  const content = (
    <Container>
        <table className="w-full border-collapse text-left text-sm/5 max-sm:hidden">
          ...
          <thead>
            <tr>
              <th className="sticky top-(--scroll-padding-top) bg-olive-100 py-5 pr-3 text-base/7 font-medium text-oxblood">
                Compare features
              </th>
              {plans.map((plan, index) => (
                <th
                  key={index}
                  className="sticky top-(--scroll-padding-top) bg-olive-100 p-3 text-center font-semibold text-oxblood"
                >
                  {plan}
                </th>
              ))}
            ...
            <ElTabList className="flex gap-6">
              {plans.map((plan) => (
                <button
                  key={plan}
                  type="button"
                  className="relative -mb-px flex-1 cursor-pointer border-b border-b-transparent px-2 py-6 text-sm/5 font-medium text-olive-500 aria-selected:border-olive-950 aria-selected:text-oxblood"
                >
                  {plan}
                </button>
              ))}
